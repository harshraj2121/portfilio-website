...
function Navigation() {
  ...
  return (
    <div>
        <nav className={`text-white text-xl px-[6%] h-12 bg-[#582F0E] flex items-center justify-between overflow-auto`}>          
              <div className={`${style.logofont} text-4xl font-semibold`}>Harsh raj</div>
              <div onClick={handleSidebar} className='md:hidden'><FontAwesomeIcon icon={faBars}/>
                <div className={`${sideBar ? 'block' : 'hidden'} bg-white p-2 text-black z-10 flex flex-col items-end absolute top-12 right-0`}>
                  ...
                  <button className='ml-6 md:ml-9 px-2 py-0 mt-[2px] text-sm rounded-full bg-[#c2c5aa] font-semibold text-black'><NavLink to="/hireme">Hire ME</NavLink></button>
                </div>
              </div>
              <div className='hidden md:block'>
                ...
                <button className='ml-6 md:ml-9 px-3 py-1 rounded-full bg-[#c2c5aa] font-semibold text-black'><NavLink to="/hireme">Hire ME</NavLink></button>
              </div>
        </nav>
    </div>
  )
